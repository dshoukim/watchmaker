import { pgTable, foreignKey, unique, check, uuid, text, timestamp, integer, jsonb, pgPolicy, bigint, boolean, date, serial, primaryKey } from "drizzle-orm/pg-core"
...
export const watchPartyContent = pgTable("watch_party_content", {
	...
	foreignKey({
			...
			name: "watch_party_content_added_by_fkey"
		}),
...
export const watchPartySwipes = pgTable("watch_party_swipes", {
	...
	foreignKey({
			...
			name: "watch_party_swipes_content_id_fkey"
		}).onDelete("cascade"),
	foreignKey({
			columns: [table.userId],
			foreignColumns: [profiles.id],
			name: "watch_party_swipes_user_id_fkey"
		}),
...
export const contentRatings = pgTable("content_ratings", {
	...
	foreignKey({
			columns: [table.contentId],
			foreignColumns: [watchPartyContent.id],
			name: "content_ratings_content_id_fkey"
		}).onDelete("cascade"),
	foreignKey({
			columns: [table.userId],
			foreignColumns: [profiles.id],
			name: "content_ratings_user_id_fkey"
		}),
...
export const streamingServices = pgTable("streaming_services", {
	...
	id: bigint("id", { mode: "number" }).primaryKey().generatedByDefaultAsIdentity({ name: "streaming_services_id_seq", startWith: 1, increment: 1, minValue: 1, /* maxValue: 9223372036854775807, */ cache: 1 }),
...
export const profiles = pgTable("profiles", {
	...
	// foreignKey({
	// 		columns: [table.id],
	// 		foreignColumns: [users.id], // This 'users' refers to public.users with an integer id
...
export const content = pgTable("content", {
	...
	unique("content_tmdb_id_unique").on(table.tmdbId),
]);
...
export const wishlist = pgTable("wishlist", {
	...
	primaryKey({ columns: [table.userId, table.contentId], name: "wishlist_user_id_content_id_pk"}),
]);
...
export const ratedContent = pgTable("rated_content", {
	...
	primaryKey({ columns: [table.userId, table.contentId], name: "rated_content_user_id_content_id_pk"}),
]);
